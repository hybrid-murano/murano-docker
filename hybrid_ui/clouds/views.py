...
class Wizard(views.ModalFormMixin, LazyWizard):
    template_name = 'clouds/wizard_create.html'
    do_redirect = False
    environment_id = env_tables.get_cloud_id()
    ...
    def done(self, form_list, **kwargs):
        ...
        fail_url = reverse("horizon:admin:clouds:index")
        try:
            env_url = reverse('horizon:admin:clouds:index')
            srv = api.service_create(self.request, self.environment_id, attributes)
        except exc.HTTPForbidden:
            msg = _("Sorry, you can't add task right now.")
            exceptions.handle(self.request, msg, redirect=fail_url)
        except Exception:
            message = _('Adding task failed.')
            LOG.exception(message)
            exceptions.handle(self.request, message, redirect=fail_url)
        else:
            message = _("The '{0}' task successfully added.").format(app_name)
            LOG.info(message)
...
class CloudView(tabs.TabbedTableView):
    ...
    def get_redirect_url():
        return reverse_lazy("horizon:admin:clouds:index")
...
class DetailServiceView(tabs.TabbedTableView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["custom_breadcrumb"] = [(_("Clouds"), CloudView.get_redirect_url()),]
        return context

    def get_data(self):
        service_id = self.kwargs['service_id']
        self.environment_id = env_tables.get_cloud_id()
        try:
...
class DeploymentDetailsView(tabs.TabbedTableView):
    ...
    page_title = _('Clouds at {{ deployment_start_time }}')
