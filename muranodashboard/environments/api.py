...
def environment_get(request, environment_id):
    if hasattr(request, 'env') and request.env.has_key(environment_id):
        return request.env[environment_id]
    ...
    request.env = getattr(request, 'env', {})
    request.env[environment_id] = env
    return env
...
def deployments_list(request, environment_id):
    if hasattr(request, 'deployments') and request.deployments.has_key(environment_id):
        return request.deployments[environment_id]
    ...
    request.deployments = getattr(request, 'deployments', {})
    request.deployments[environment_id] = deployments
    return deployments


def deployment_reports(request, environment_id, deployment_id):
    if hasattr(request, 'deployments') and request.deployments.has_key((environment_id, deployment_id)):
        return request.deployments[(environment_id, deployment_id)]
    ...
    request.deployments = getattr(request, 'deployments', {})
    request.deployments[(environment_id, deployment_id)] = reports
    return reports
