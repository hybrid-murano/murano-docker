...
class Service(object):
    ...
    def evaluate(self, spec):
        if isinstance(spec, yaql_expression.YaqlExpression):
            return spec.evaluate(data=self.cleaned_data, context=self.context)
        elif isinstance(spec, dict):
            items = []
            for k, v in six.iteritems(spec):
                items.append((k, self.evaluate(v)))
            return dict(items)
        elif isinstance(spec, list):
            return [self.evaluate(_spec) for _spec in spec]
        else:
            return spec
    ...
    def get_data(self, form_name, expr, data=None):
        ...
        if isinstance(expr, yaql_expression.YaqlExpression):
            return data != {}, data and expr.evaluate(data=data, context=self.context)
        else:
            return True, self.evaluate(expr)
