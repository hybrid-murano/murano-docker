...
class EngineClient(object):
    def __init__(self, transport):
        ...
        self._client = rpc.RPCClient(transport, client_target, timeout=15)
        broadcast_target = target.Target('murano', 'tasks', fanout=True)
        self._broadcast = rpc.RPCClient(transport, broadcast_target, timeout=15)

    def handle_task(self, task):
        return self._client.cast({}, 'handle_task', task=task)

    def broadcast_task(self, task):
        return self._broadcast.cast({}, 'handle_task', task=task)
