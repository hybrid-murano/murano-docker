...
class TaskExecutor(object):
    ...
    def _execute(self, pkg_loader):
        ...
        try:
            LOG.info('Invoking pre-cleanup hooks')
            self.session.start()
            executor.cleanup(self._model)
        except Exception as e:
            self.exception_result(e, obj, '<GC>')
        finally:
            LOG.info('Invoking post-cleanup hooks')
            self.session.finish()
        ...
        if self.action:
            try:
                LOG.info('Invoking pre-execution hooks: {0}'.format(self.action['method']))
                self.session.start()
                try:
    ...
    def _delete_trust(self):
        ...
        if trust_id:
            auth_utils.delete_trust(self._session.trust_id, self._session.token, self._session.project_id)
